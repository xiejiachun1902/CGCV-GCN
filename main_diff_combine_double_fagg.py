...
class Processor():
    ...
    def train(self, epoch, save_model=False):
        ...
        for batch_idx, (data_m, data_p, label, feature, index) in enumerate(process):
            ...
            #     with open("data_m.txt", "w") as f:
            #         f.write(str(data_m))
            #     with open("data_p.txt", "w") as f:
            #         f.write(str(data_p))
            ...
            # data_m = data_m.float().cuda(self.output_device).detach()
            # data_p = data_p.float().cuda(self.output_device).detach()
            # label = label.long().cuda(self.output_device).detach()
            if train_mode == 'MLL': label = label.to(torch.float32)
            # feature = Variable(feature.float().cuda(self.output_device), requires_grad=False)
            feature = feature.float().cuda(self.output_device).detach()
