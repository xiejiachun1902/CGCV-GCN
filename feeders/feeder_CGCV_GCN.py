...
class Feeder(Dataset):
    def __init__(self, data_k_path, data_p_path, label_path, feature_path,
    ...
    def get_mean_map(self):
        data_k = self.data_k
        N, C, T, V, M = data_k.shape
        # self.mean_map_k = data_k.mean(axis=2, keepdims=True).mean(axis=4, keepdims=True).mean(axis=0)+1e-6
        # self.std_map_k = data_k.transpose((0, 2, 4, 1, 3)).reshape((N * T * M, C * V)).std(axis=0).reshape((C, 1, V, 1))+1e-6
        self.mean_map_k = data_k.mean(axis=0)
        self.std_map_k = data_k.std(axis=0) + 1e-6
...
class FeederDataset(Dataset):
    def __init__(self, data_p, data_k, feature, label, random_choose=False, random_shift=False, random_move=False,
                 window_size=-1, normalization=False, debug=False, use_mmap=True):
        self.data_p = data_p
        self.data_k = data_k
...
class FeederSplit(Dataset):
    def __init__(self, train_data_k_path, train_data_p_path, train_label_path, train_feature_path,
                 test_data_k_path, test_data_p_path, test_label_path, test_feature_path,
                 train_ratio=0.9, val_ratio=0, test_ratio=0.1,
                 random_choose=False, random_shift=False, random_move=False,
                 window_size=-1, normalization=False, debug=False, use_mmap=True):
        self.debug = debug
        self.train_data_k_path = train_data_k_path
        self.train_data_p_path = train_data_p_path
        self.train_label_path = train_label_path
        self.train_feature_path = train_feature_path
        self.test_data_k_path = test_data_k_path
        ...
        if train_ratio + test_ratio != 1: test_idxes = np.random.choice(test_idxes, int(len(test_idxes) * test_ratio / (
                    1 - train_ratio)), replace=False)
        self.train_set = FeederDataset(self.data_p[train_idxes], self.data_k[train_idxes], self.feature[train_idxes],
                                       self.label[train_idxes], self.random_choose, self.random_shift, self.random_move,
                                       self.window_size, self.normalization, self.debug, self.use_mmap)
        self.test_set = FeederDataset(self.data_p[test_idxes], self.data_k[test_idxes], self.feature[test_idxes],
                                      self.label[test_idxes], self.random_choose, self.random_shift, self.random_move,
                                      self.window_size, self.normalization, self.debug, self.use_mmap)
    ...
    def load_data(self):
        ...
        # load data
        if self.use_mmap:
            self.train_data_k = np.load(self.train_data_k_path, mmap_mode='r')
        ...
        self.data_k = np.concatenate([self.train_data_k, self.test_data_k], axis=0)
